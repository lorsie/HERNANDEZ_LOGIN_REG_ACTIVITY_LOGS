...
function searchForAnApplicant($pdo, $searchQuery) {
	...
	$sql = "SELECT * FROM search_applicant WHERE 
			CONCAT(first_name, last_name, position, gender, age, email, contact_number, address, hire_date, date_added) 
			LIKE ?";
	$stmt = $pdo->prepare($sql);
...
function insertNewApplicant($pdo, $first_name, $last_name, $position, 
    $gender, $age, $email, $contact_number, $address, $hire_date, $added_by) {
    ...
    $executeQuery = $stmt->execute([
        ...
        $gender, $age, $email, 
        $contact_number, $address, $hire_date, $added_by,
    ]);
    ...
    if ($executeQuery) {
		...
		$insertAnActivityLog = insertAnActivityLog($pdo, "INSERT", $getApplicantID['id'], 
			...
			$getApplicantID['position'], $getApplicantID['gender'], $getApplicantID['age'], $getApplicantID['email'], $getApplicantID['contact_number'], $getApplicantID['address'], $_SESSION['username']);
...
function editApplicant($pdo, $first_name, $last_name, $position, 
    $gender, $age, $email, $contact_number, $address, $hire_date, $id, $modified_by) {
    ...
    $executeQuery = $stmt->execute([
        $first_name, $last_name, $position, 
        $gender, $age, $email, 
        $contact_number, $address, $hire_date, $modified_by, $id
    ]);
    ...
    if ($executeQuery) {
		...
		$insertAnActivityLog = insertAnActivityLog($pdo, "UPDATE", $getApplicantID['id'], 
			$getApplicantID['first_name'], $getApplicantID['last_name'], 
			$getApplicantID['position'], $getApplicantID['gender'], $getApplicantID['age'], $getApplicantID['email'], $getApplicantID['contact_number'], $getApplicantID['address'], $getApplicantID['hire_date'], $_SESSION['username']);
...
function insertAnActivityLog($pdo, $operation, $id, $first_name, 
    $last_name, $position, $gender, $age, $email, $contact_number, $address, $username) {

    $sql = "INSERT INTO activity_logs (operation, id, first_name, 
        last_name, position, gender, age, email, contact_number, address, hire_date, username) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    $stmt = $pdo->prepare($sql);
    $executeQuery = $stmt->execute([
        $operation, $id, $first_name, 
        $last_name, $position, $gender, $age, $email, $contact_number, $address, $hire_date, $username
    ]);
